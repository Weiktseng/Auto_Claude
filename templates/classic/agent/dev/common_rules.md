...
7. **測試品質規則**
   ...
   **⚠️ 本專案對上面最後一段的唯一改動** — 原文要求 "confirm them with the user"，但本迴圈人類不在場：
   ...
   - 本專案的 **pre-agreed seam = `agent/spec.txt` 裡某條需求編號（`FR-*` / `NFR-*`）所描述的對外行為**。測試寫在那個介面上。
   - **spec 沒有對應需求編號的 seam，不要自己開測試。** 想加 → 寫進 `agent/comms/human_reply.md` 提案，繼續做別的。
   - 每個測試的名字或註解要標出它對應哪條需求編號（例：`# FR-CHAT-01`），讓人類事後能對帳。
